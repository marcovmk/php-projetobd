<?php
    ...
    function inserirVeiculo($placa, $modelo, $ano){
        try{ 
            $sql = "INSERT INTO VEICULO (PLACA, MODELO, ANO)VALUES (:placa, :modelo, :ano)";
            $conexao = conectarBanco();
            $stmt = $conexao->prepare($sql);
    ...
    function alterarVeiculo($placa, $modelo, $ano, $cod_veiculo){
        try{ 
            $sql = "UPDATE VEICULO SET placa = :placa, modelo = :modelo, ano = :ano WHERE cod_veiculo = :cod_veiculo";
            $conexao = conectarBanco();
            $stmt = $conexao->prepare($sql);
            $stmt->bindValue(":placa", $placa);
            $stmt->bindValue(":modelo", $modelo);
    ...
    function consultarViagemID($cod_viagem){
        try{ 
            $sql = "SELECT * FROM VIAGEM WHERE $cod_viagem = :$cod_viagem";
            $conexao = conectarBanco();
            $stmt = $conexao->prepare($sql);
